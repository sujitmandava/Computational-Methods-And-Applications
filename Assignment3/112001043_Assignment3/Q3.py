    ...
    def sampleSymmetric(self):
        for i in range(self.dimension):
            for j in range(i, self.dimension):
                if i == j:
                    self.squareMatrix[i][j] = uniform(
                        self.dimension, self.dimension+10)
                else:
...
def visualizeConvergence(dimension, jErrors, gErrors):
    numIterations = len(jErrors)

    x = list(range(1, numIterations + 1))
    plt.title(
        f"Rate of convergence of Jacobi and Gauss-Seidel methods\n({numIterations} iterations, {dimension}x{dimension} matrix)"
    )
    plt.xlabel("Number of iterations")
    plt.ylabel("Error : ||Ax⁽ᵏ⁾ - b||₂")
    plt.plot(x, jErrors, c="b", label="Jacobi Method")
    plt.plot(x, gErrors, c="r", label="Gauss-Seidel Method")
    plt.legend()
    plt.grid()
    plt.show()
...
if __name__ == '__main__':
    ...
    jError, jSol = s.jSolve([12, 23, 3, 42], numIterations)
    gError, gSol = s.gSolve([12, 23, 3, 42], numIterations)
    
    visualizeConvergence(s.dimension, jError, gError)
