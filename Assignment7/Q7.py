...
class Polynomial:
    ...
    def findRootInInterval(self, a, b, f, e=1e-3):
        points = []
        X = np.arange(a, b, 0.1)
        for x in X:
            points.append([x, f(x)])

        bestPoly = self.bestFitFunc(points=points, n=12)
        # bestPoly.show(-5, 5)

        roots = bestPoly.printRoots(e)

        for r in roots:
            if a <= abs(r) and abs(r) <= b:
                print(f"Root: {abs(r)}")


if __name__ == "__main__":
    def f(x): return np.sin(x)
    # print(abs(complex(2 * np.cos(np.pi/2), 2 * np.sin(np.pi/2))))
    Polynomial([]).findRootInInterval(-4, 4, f=f)
